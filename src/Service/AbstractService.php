...
use Lendable\Interview\Interpolation\{
    ...
    Component\Exception\CommonException,
    Model\LoanModel
};
 ...
 */
abstract class AbstractService implements CalculatorInterface
{

    /**
     * Error
     */
    const ERROR_CLASS_NOT_FOUND_MASK = 'Unable to find service: {class}';
    ...
    /**
     * Factory. Returns back a service object by name
     *
     ...
     */
    public static function factory(string $serviceName): CalculatorInterface
    {
        if (isset(self::$_serviceInstances[$serviceName]) === true) {
            return self::$_serviceInstances[$serviceName];
        }

        $className = sprintf(
            'Lendable\\Interview\\Interpolation\\Service\\%sService',
            ucfirst($serviceName)
        );

        if (class_exists($className) === false) {
            throw new CommonException(
                self::ERROR_CLASS_NOT_FOUND_MASK,
                [
                    'class' => $className,
                ]
            );
        ...
        self::$_serviceInstances[$serviceName] = $serviceInstance;
     ...
     * @return float
     */
    protected function calculateFee(LoanModel $model): float
    {
